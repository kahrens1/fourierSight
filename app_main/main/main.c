...
static uint8_t num_frames_captured = 0;
...
static void task_fft(void *args){ 
    ...
    while(1){
        ...
        uint64_t *fft_mags = (uint64_t *)malloc(FFT_SIZE*sizeof(uint64_t)); // TODO: Fix so this can be done in place (remove this allocation)
...
static void task_uart_send_fft_data(void *args){ 
    ...
    while(1){
        ...
        uart_write_bytes(UART_NUM_0,(const void *)fft_mags_ptr,FFT_SIZE*sizeof(uint64_t)); 
        free(fft_mags_ptr);
    }
...
void app_main(void){
    ...
    fft_data_queue = xQueueCreate(FFT_QUEUE_SIZE,sizeof(uint64_t *));
