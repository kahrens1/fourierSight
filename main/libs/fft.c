#include"fft.h"
    ...
    static const uint16_t bitRev512[512] = {
        0,256,128,384,
        64,320,192,448,
        32,288,160,416,
        96,352,224,480,
        ...
        static const uint8_t bitRev256[256] = {
            0,128,64,192,
            32,160,96,224,
            16,144,80,208,
            48,176,112,240,
    ...
    static const uint8_t bitRev64[64] = {
        0,32,16,48,
        8,40,24,56,
        4,36,20,52,
        12,44,28,60,
    ...
    static const uint8_t bitRev32[32] = {
        0,16,8,24,
        4,20,12,28,
        2,18,10,26,
        6,22,14,30,
...
_Bool fft_init(fft_instance_t *fft_init, uint16_t numPoints, _Bool bRevFlag){
    ...
    fft_init->bBitRevFlag = bBitRevFlag;
...
void fft_compute(fft_instance_t *fft_init, complex_t *data){
    ...
    for(uint8_t stage_idx = 0; stage_idx < fft_init->numStages; stage_idx++){
        ...
        numBfs = 2^stage_idx;
        bf_span = 2^stage_idx;
        blk_step = 2^(stage_idx + 1);
        bf_step = 1;
        //Loop over blocks
        for(uint8_t blk_idx = 0; blk_idx < numBlocks; blk_idx++){
            blk_ptr = blk_idx*blk_step;
            // Loop over BFs in the block
            for(uint8_t bf_idx = 0; bf_index < numBfs; bf_idx++ ){
               // tf =  // TF Lookup 
                uint16_t in1 = blk_ptr + bf_idx*bf_step; 
